...
public class Jogo {
    ...
    protected void fantasy(Heroi jogadorAtual) throws InterruptedException {
        ...
        NPC megalodon = new NPC("Megalodon", 300, 50, 200);
        ...
        NPC guardiaoMortoKades = new NPC("Guardião Morto Kades", 500, 80, 1000);
        ...
        // Contexto Diferente Surpresa Na Sala 5
        ...
        // Criar Armas
        ItemHeroi faca = new ArmaPrincipal("Faca de Aprendiz", 0, 15, 25);
        ItemHeroi arco = new ArmaPrincipal("Arco", 30, 40, 150);
        ItemHeroi arcoCaca = new ArmaPrincipal("Arco de Caça", 60, 100, 200);
        ItemHeroi espada = new ArmaPrincipal("Espada", 30, 40, 150);
        ItemHeroi laminaSagrada = new ArmaPrincipal("Lâmina Sagrada", 70, 100, 200);
        ItemHeroi punhal = new ArmaPrincipal("Punhal", 30, 40, 150);
        ItemHeroi gladius = new ArmaPrincipal("Gladius", 60, 90, 200);
        ItemHeroi bastao = new ArmaPrincipal("Bastão", 30, 40, 150);
        ItemHeroi cajadoFeiticeiro = new ArmaPrincipal("Cajado do Feiticeiro", 60, 100, 200);
        ItemHeroi machado = new ArmaPrincipal("Machado", 40, 40, 150);
        ItemHeroi machado2Maos = new ArmaPrincipal("Machado de Duas Mãos", 80, 100, 200);
        ItemHeroi cajadoSarcerdote = new ArmaPrincipal("Cajado do Sarcerdote", 60, 100, 200);
        ...
        Consumivel pilulaDeCombate = new ConsumivelCombate("Pilura de Combate", 30, 100);
        Consumivel pocaoDoFuror = new ConsumivelCombate("Poção do Furor", 40, 120);
        Consumivel bombaAcida = new ConsumivelCombate("Bomba Ácida", 50, 150);
        Consumivel venenoMortal = new ConsumivelCombate("Veneno Mortal", 60, 180);

        // Instancia de Poção para restaurar Vida:
        Consumivel churrascoDeSelvagem = new Pocao("Churrasco de Selvagem", 30, 15, 15);
        Consumivel coquetelUivante = new Pocao("Coquetel Uivante", 35, 20, 25);
        Consumivel carneAoVinho = new Pocao("Carne ao Vinho", 40, 25, 25);
        Consumivel frutoDeYggdrasil = new Pocao("Fruto de Yggdrasil", 45, 30, 35);
        Consumivel bolinhoDivino = new Pocao("Bolinho Divino", 50, 40, 50);
        ...
        System.out.println("Em um mundo repleto de magia e mistério, " +
                "um jovem aprendiz chamado \n" + jogadorAtual.getNomeEntidade() + " " +
                "embarca em uma jornada épica em busca de poder e glória. " +
    ...
    protected void pecorrerJogoInteiro(int indexAtualSalaDoJogo, Heroi jogadorAtual) throws InterruptedException {
        ...
        int opcaoHeroiUsarPocao;
        boolean opcaoInvalidaPocao = true;
        boolean opcaoInvalida = true;
        boolean opcaoHeroiUsarPocaoFalse = true;
        boolean opcaMorteHeroi = true;


        //Laço para criar validação de morte do personagem:
        while (jogadorAtual.getVidaEntidade() > 0 && opcaMorteHeroi) {
            ...
            for (NPC npcAtual : salaAtual.getInimigoNaSala()) {
                System.out.println("###################################################################");
                System.out.println("\t\t\tVai Enfrentar o(a) " + npcAtual.getNomeEntidade() + "; HP = " + npcAtual.getVidaEntidade() + "; FOR = " + npcAtual.getForcaEntidade());
                System.out.println("###################################################################");
                Thread.sleep(1000);
                String mensagem = mensagemBatalhaPorNPC(npcAtual);
                System.out.println(mensagem);
                System.out.println("\t\t\t»» Prepare-se Para Enfrentá-lo ««");
                Thread.sleep(1000);
                jogadorAtual.atacar(npcAtual);

                if (jogadorAtual.getVidaEntidade() <= 0) {
                    System.err.println("\t\t\t****** Game Over ******");
                    opcaMorteHeroi = false;
                }
            ...
            // Invocação do Método para Herói usar Poção:
            System.out.println("*********************************************************************************");
            System.out.println("\t\t\t\t» Hora de Recuperar o Fôlego !! «");
            System.out.println("*********************************************************************************");
            ...
            do {
                ...
                System.out.print("» Deseja Utilizar Poção para Restaurar o HP? ");

                try {
                    opcaoHeroiUsarPocao = input.nextInt();
                } catch (InputMismatchException mismatchException) {
                    System.err.println("Erro: Entrada inválida. Certifique-se de digitar um número inteiro.");
                    input.next();
                    opcaoHeroiUsarPocao = 0;
                }

                if (opcaoHeroiUsarPocao == 1) {
                    salaParaHeroiUsarPocao(jogadorAtual);
                } else if (opcaoHeroiUsarPocao == 2) {
                    opcaoInvalidaPocao = false;
                } else {
                    System.out.println("Opção inválida. Por favor, escolha 1 para comprar ou 2 para sair.");
                }
            } while (!opcaoHeroiUsarPocaoFalse);

            // ForEach para imprimir salas conectadas:
        ...
        if (jogadorAtual.getVidaEntidade() <= 0 && !opcaMorteHeroi) {
            System.out.println("[ 1 ] - Sim");
            System.out.println("[ 2 ] - Não");
            System.out.println("Deseja Jogar Novamente?");
            int opcaoContinuarJogando = input.nextInt();
            if (opcaoContinuarJogando == 1) {
                pecorrerJogoInteiro(0, jogadorAtual);
            }
        }
    }
     ...
     * @param jogadorAtual - Heroi / jogadorAtual
     */
    private void salaParaHeroiUsarPocao(Heroi jogadorAtual) throws InterruptedException {
        ...
        System.err.println(" Uma pequena Porta no meio de tantos desafios, se abre para dar um pouco de descanso para o Aventureiro!! ");
        Thread.sleep(500);
        ...
        for (numContadorUsoPocao = 1; numContadorUsoPocao <= 3; numContadorUsoPocao++) {
            ...
            for (numIndice = 0; numIndice < inventario.size(); numIndice++) {
                ItemHeroi itemHeroi = inventario.get(numIndice);
            ...
            if (opcaoContador == 0) {
                System.err.println("\tInventário de Poções Vazio!");
                Thread.sleep(150);
                System.out.println("***************************************************");
                Thread.sleep(250);
                ...
                Consumivel pocaoEscolhida = (Consumivel) inventario.get(opcaoCompraVendedor - 1);
                if (pocaoEscolhida instanceof Pocao) {
    ...
    protected void boasVindas(int indexAtualSalaDoJogo) throws InterruptedException {
        if (indexAtualSalaDoJogo == 0) {
            ...
            Thread.sleep(150);
            System.out.println("########################################################################################");
        } else if (indexAtualSalaDoJogo == 1 || indexAtualSalaDoJogo == 12) {
            //Vendedor
            System.out.println("########################################################################################");
    ...
    protected Heroi criarPersonagem() throws InterruptedException {
        // Invocação do Nome do jogo:
        nomeJogoFantasy();

        // Impor Scanner inpu para Entrada de dados:
        Scanner input = new Scanner(System.in);
    ...
    /**
     * Método para imprimir no console Nome do Jogo:
     */
    private void nomeJogoFantasy() {
        String yellowText = "███████╗ █████╗ ███╗   ██╗████████╗ █████╗ ███████╗██╗   ██╗\n" +
                "██╔════╝██╔══██╗████╗  ██║╚══██╔══╝██╔══██╗██╔════╝╚██╗ ██╔╝\n" +
                "█████╗  ███████║██╔██╗ ██║   ██║   ███████║███████╗ ╚████╔╝ \n" +
                "██╔══╝  ██╔══██║██║╚██╗██║   ██║   ██╔══██║╚════██║  ╚██╔╝  \n" +
